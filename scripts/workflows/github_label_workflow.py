...
def get_labels(pr_number):
    """ getting PR's labels """
    r = requests.get(f"{REPO_URL}/issues/{pr_number}/labels", headers=API_HEADERS)
...
def remove_labels(pr_number, labels):
    ...
    for label in labels:
        r = requests.delete(
            f"{REPO_URL}/issues/{pr_number}/labels/{label}", headers=API_HEADERS
        )
        print(r.json())
...
def manage_labels(pr_number, adding_labels, available_labels):
    ...
    removing_labels = set(existing_labels) & (
        set(available_labels) - set(adding_labels)
    )
...
def label_pr_by_size(pr_number):
    """Check PR size"""
    all_size_labels = ["size-XS", "size-S", "size-M", "size-L", "size-XL"]
    ...
    manage_labels(pr_number, adding_labels=[label], available_labels=all_size_labels)
...
def label_pr_by_state(pr_number):
    ...
    all_state_labels = ["wip", "requires-review", "changes-requested", "awaiting-apply"]
    ...
    if reviews and reviews[-1] in state_mapping:
        label = state_mapping[reviews[-1]]

    manage_labels(pr_number, adding_labels=[label], available_labels=all_state_labels)
